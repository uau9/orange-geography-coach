(function registerTideFeature(root) {
  ...
  function renderLab({ lab, scenario, item, scenarioIndex = 0 }) {
    return `<div class="topic-meta">自然地理 · 宇宙中的地球 · ${esc(scenario.id)}</div><h2 class="page-title">潮汐周期与月相实验室</h2><p class="page-subtitle">先把月相还原成日地月方向，再判断引潮作用如何组合；最后区分“总体潮差规律”和“当地准确潮位”。</p><section class="tide-scene-bar"><div><span>情境 ${scenarioIndex + 1}/${lab.scenarios.length}</span><strong>${esc(item.phase)} · 日月夹角 ${esc(item.angle_deg)}°</strong></div><div><span>本轮任务</span><strong>${esc(scenario.challenge)}</strong></div></section><section class="card tide-card"><div class="tide-layout"><div><div class="solar-model-head"><div><span class="pill orange">日地月俯视模型</span><h3>月相—引潮方向—潮差—局地边界</h3></div><span class="pill">先预测后解锁</span></div>${model(item, false)}<div class="activity-observation-list">${scenario.observations.map((observation, index) => `<div><span>观测 ${index + 1}</span><strong>${esc(observation)}</strong></div>`).join("")}</div><p class="motion-hint">图示采用高中理想模型，只表达日地月相对方向；不按真实距离和大小绘制，也不预测某港口的准确潮时与潮高。潮型、潮差与周期答案提交前隐藏。</p></div><form id="tide-form" class="tide-prediction-panel"><div class="notice">判断链：月相 → 日月方向 → 引潮作用组合 → 潮型 → 潮差 → 周期与局地边界。</div><fieldset><legend>1. 最符合该月相的几何关系是：</legend>${radioGroup("tide-geometry", scenario.choices.geometry, `${scenario.id}-geometry`)}</fieldset><fieldset><legend>2. 该阶段最合理的潮型是：</legend>${radioGroup("tide-type", scenario.choices.tide_type, `${scenario.id}-type`)}</fieldset><fieldset><legend>3. 潮差的总体特征是：</legend>${radioGroup("tide-range", scenario.choices.range, `${scenario.id}-range`)}</fieldset><fieldset><legend>4. 最合理的周期判断是：</legend>${radioGroup("tide-cycle", scenario.choices.cycle, `${scenario.id}-cycle`)}</fieldset><fieldset><legend>5. 最稳妥的证据结论是：</legend>${radioGroup("tide-conclusion", scenario.choices.conclusion, `${scenario.id}-conclusion`)}</fieldset><label class="field-label" for="tide-reasoning">写出判断链</label><textarea id="tide-reasoning" name="tide-reasoning" placeholder="请依次写月相位置、日月方向、潮型、潮差，再说明能否据此预报某港口的准确潮时潮高。"></textarea><button class="btn orange motion-submit" type="submit">提交五步预测，解锁潮差模型</button></form></div></section>`;
  }
