(function registerAxialTiltFeature(root) {
  ...
  function modelDynamic(result, unlocked) {
    ...
    const zones = unlocked ? `
      ...
      <g class="axial-zone-lock">
        ...
        <text x="375" y="159" text-anchor="middle">默认可见</text>
      </g>`;
    return `
      ${zones}
      <g class="axial-earth">
  ...
  function renderLab({ lab, scenario, scenarioIndex = 0 }) {
    return `
      ...
      <section class="card axial-tilt-card"><div class="axial-tilt-layout"><div><div class="solar-model-head"><div><span class="pill orange">反事实地球模型</span><h3>倾角—回归线—极圈—五带</h3></div><span class="pill">0°—45°理想推演</span></div>${model(scenario, true, scenario.target_tilt_deg, lab.facts)}<p class="motion-hint">${esc(lab.model_note)} 回归线、极圈和五带边界默认可见。</p></div><form id="axial-tilt-form" class="axial-prediction-panel"><div class="notice">判断链：ε → 回归线纬度 ε → 极圈纬度 90°−ε → 热带总宽 2ε → 每个温带宽 90°−2ε。</div><fieldset><legend>1. 回归线纬度：</legend><output id="axial-tropic-output" class="axial-range-output">23.5°</output><input id="axial-tropic-prediction" name="axial-tropic" type="range" min="0" max="45" step="0.5" value="23.5" aria-label="预测回归线纬度"/></fieldset><fieldset><legend>2. 极圈纬度：</legend><output id="axial-polar-output" class="axial-range-output">66.5°</output><input id="axial-polar-prediction" name="axial-polar" type="range" min="45" max="90" step="0.5" value="66.5" aria-label="预测极圈纬度"/></fieldset><fieldset><legend>3. 南北回归线之间的热带总宽：</legend><output id="axial-tropical-width-output" class="axial-range-output">47°</output><input id="axial-tropical-width-prediction" name="axial-tropical-width" type="range" min="0" max="90" step="1" value="47" aria-label="预测热带总宽"/></fieldset><fieldset><legend>4. 每个温带的纬度宽度：</legend><output id="axial-temperate-output" class="axial-range-output">43°</output><input id="axial-temperate-prediction" name="axial-temperate" type="range" min="0" max="90" step="1" value="43" aria-label="预测每个温带宽度"/></fieldset><fieldset><legend>5. 与当前23.5°相比，五带怎样变化？</legend>${choices(lab.choices.zone_change)}</fieldset><label class="field-label" for="axial-reasoning">判断链（选填）</label><textarea id="axial-reasoning" name="axial-reasoning" placeholder="例如：ε增大，回归线纬度增大；极圈纬度=90°−ε而减小，因此热带和寒带变宽、温带变窄。"></textarea><button class="btn orange motion-submit" type="submit">提交五步预测</button></form></div></section>
    `;
  }
