(function registerHabitabilityFeature(root) {
  ...
  function renderLab({ lab, scenario, scenarioIndex = 0 }) {
    ...
    return `<div class="topic-meta">自然地理 · 宇宙中的地球 · ${esc(scenario.id)}</div><h2 class="page-title">地球宜居条件对照实验室</h2><p class="page-subtitle">像做对照实验一样分变量：先比日距和辐射，再比大气与气压，最后才讨论温度、液态水和生命证据。</p><section class="habitability-scene-bar"><div><span>情境 ${scenarioIndex + 1}/${lab.scenarios.length}</span><strong>${esc(scenario.challenge)}</strong></div><div><span>对照组</span><strong>${esc(bodyA.name)} vs ${esc(bodyB.name)}</strong></div></section><section class="card habitability-card"><div class="habitability-layout"><div><div class="solar-model-head"><div><span class="pill orange">变量对照</span><h3>轨道—大气—温度—液态水</h3></div><span class="pill">先预测后解锁</span></div>${model(lab, scenario, false, 1)}<p class="motion-hint">${esc(lab.model_note)} 温度、地表水和结论提交前隐藏。</p></div><form id="habitability-form" class="habitability-prediction-panel"><div class="notice">判断链：日距 → 接收太阳辐射 → 大气总量与成分 → 温室效应和温度 → 水的相态 → 证据边界。</div><fieldset><legend>1. 接收太阳辐射较多的是：</legend>${radioGroup("habitability-solar", compareChoices)}</fieldset><fieldset><legend>2. 地表气压较高的是：</legend>${radioGroup("habitability-pressure", compareChoices)}</fieldset><fieldset><legend>3. 综合日距和大气信息，当前表面温度更接近液态水温度窗口的是：</legend>${radioGroup("habitability-temperature", [bodyA.name, bodyB.name, "两者都不接近"])}</fieldset><fieldset><legend>4. 当前有地表长期稳定液态水证据的是：</legend>${radioGroup("habitability-water", [bodyA.name, bodyB.name, "两者都缺少地表长期稳定液态水"])}</fieldset><fieldset><legend>5. 由这组对照最合理的推论是：</legend>${radioGroup("habitability-inference", scenario.inference_choices)}</fieldset><label class="field-label" for="habitability-reasoning">写出判断链</label><textarea id="habitability-reasoning" name="habitability-reasoning" placeholder="例如：先按日距判断太阳辐射，再比较气压和温室效应；有水冰不等于有稳定液态水，更不能直接证明生命。"></textarea><button class="btn orange motion-submit" type="submit">提交五步预测，解锁表面证据</button></form></div></section>`;
  }
