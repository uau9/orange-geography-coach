...
let catalog = { topics: [], questions: [], paperReviews: [], retests: [] };
...
function normalizeState(parsed) {
  const normalized = { ...defaultState(), ...parsed, version: "0.2.0" };
...
function loadState() {
  try {
    ...
    return parsed && ["0.1.0", "0.2.0"].includes(parsed.version) ? normalizeState(parsed) : defaultState();
  } catch {
    return defaultState();
  }
}
...
function completedToday() {
  ...
  return state.attempts.filter((attempt) => attempt.submitted_at.startsWith(day)).length;
}
...
function renderToday() {
  ...
  app.innerHTML = `
    <h2 class="page-title">今天，先做一题</h2>
    <p class="page-subtitle">不追求题量，先留下完整的思考证据。AI负责提出诊断候选，家长负责确认。</p>
    <section class="card hero-card">
      ...
      <div class="btn-row"><button class="btn orange" data-action="start-next">开始诊断</button><button class="btn secondary" data-action="goto" data-route="parent">查看家长页</button></div>
    </section>
    <section class="stat-grid">
      <div class="stat"><strong>${completedToday()}</strong><span>今日完成</span></div>
      <div class="stat"><strong>${state.attempts.length}</strong><span>累计作答</span></div>
      <div class="stat"><strong>${state.attempts.filter((attempt) => attempt.is_correct).length}</strong><span>答对次数</span></div>
    </section>
    <section class="card">
      <h3>最近记录</h3>
...
function renderRetest() {
  ...
  app.innerHTML = `
    ...
    <section class="card">
      <div class="notice">先看变量、单位和时间尺度，再写总体趋势。提交前不显示评分点。</div>
...
function renderMastery() {
  const latestRetest = latestRetestAttempts()[0];
  const retest = getRetest(latestRetest?.retest_id) || catalog.retests[0];
  app.innerHTML = `
    ...
    ${retest ? `<section class="card"><div class="attempt-head"><div><span class="pill orange">图表专项</span><h3>${escapeHtml(retest.title)}</h3></div>${latestRetest ? `<span class="pill ${latestRetest.parent_review_status === "已掌握" ? "green" : "orange"}">${escapeHtml(latestRetest.parent_review_status)}</span>` : ""}</div><p class="small">${latestRetest ? `上次得分：${latestRetest.score ?? "待核对"}/${retest.questions.reduce((sum, question) => sum + question.max_points, 0)} · ${latestRetest.next_due_at ? `下次建议 ${formatDate(latestRetest.next_due_at)}` : "等待家长审核"}` : "来自真实试卷第19题的换情境复测。"}</p><div class="btn-row"><button class="btn orange" data-action="start-retest" data-retest-id="${escapeHtml(retest.id)}">开始复测</button></div></section>` : ""}
  `;
}
...
function renderParent() {
  ...
  app.innerHTML = `
    ...
    <section class="card"><h3>图表复测审核</h3>${retestAttempts.length ? `<div class="attempt-list">${retestAttempts.map(renderParentRetestAttempt).join("")}</div>` : `<div class="empty">橙子提交图表复测后，这里会出现评分点。</div>`}</section>
...
document.addEventListener("change", async (event) => {
  ...
  try {
    ...
    if (!["0.1.0", "0.2.0"].includes(imported.version) || !Array.isArray(imported.attempts)) throw new Error("版本不匹配");
...
function exportData() {
  const blob = new Blob([JSON.stringify({ version: state.version, exported_at: new Date().toISOString(), attempts: state.attempts, retest_attempts: state.retestAttempts }, null, 2)], { type: "application/json" });
...
async function init() {
  try {
    const [topics, questions, paperReviews, retests] = await Promise.all([
    ...
    catalog = { topics, questions, paperReviews, retests };
    render();
  } catch (error) {
