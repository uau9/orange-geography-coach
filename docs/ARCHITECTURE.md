...
- `textbook_close_reading.json` 是选择性必修1第二章第一节的逐页精读数据，保存教材18—25页映射、阅读目标、保存后解析、知识发散、迁移任务与题目分组；
...
- `textbook-close-reading.js`：教材页导航、四类折叠辅助解读、已读进度、真题/资料包分组和家长验收清单的纯渲染；
- 后续专题可继续拆分自己的渲染和计算模块。
...
- 应用资源版本当前为 `0.30.1`，集中定义在 `assets/config.js`；
...
- 第二章第一节教材第18—25页以页面图片发布；每页提供默认收起的图片解读、逐页解析、知识发散和本页总结，学生可反复展开与收起；浏览器只保存“本页已读”时间，不保存关键词、因果链或疑问；
