...
- `date_range_attempts`；
- `axial_tilt_attempts`；
- `celestial_scale_attempts`；
- `habitability_attempts`；
- `solar_activity_attempts`；
- `moon_phase_attempts`；
- `eclipse_attempts`；
- `tide_attempts`；
- `coriolis_attempts`；
- `front_weather_attempts`；
- `cyclone_system_attempts`。
- `atmosphere_reasoning_attempts`。
...
档案内的 `annotation_guide` 已包含机器可读的字段模板和批注原则。格式约束见 `schemas/learning-export.v0.24.0.schema.json`。
